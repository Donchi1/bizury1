  ...
  useEffect(() => {
    const loadFlashSales = async () => {
      try {
        ...
        // Filter products with flash sale badges
        ...
        toast({
          title: "Error",
          description: error?.message || "Failed to load flash sales",
          variant: "destructive"
        })
      } finally {
        setLoading(false)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-lg p-8 text-white mb-8">
        ...
        <p className="text-orange-100 text-lg">Lightning deals that won't last long!</p>
        <p className="text-orange-200 mt-2">{products.length} flash deals available</p>
        <div className="flex items-center space-x-2 mt-4 text-orange-100">
      ...
      <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" : "space-y-4"}>
        {sortedProducts.map((product) => (
          <Card key={product.asin} className="group hover:shadow-lg transition-shadow relative border-2 border-orange-200">
            <CardContent className={viewMode === "grid" ? "p-4" : "p-4 flex space-x-4"}>
              {/* Flash Sale Badge */}
              <Badge className="absolute top-2 left-2 bg-orange-500 text-white z-10 flex items-center animate-pulse">
      ...
      {sortedProducts.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Check back later for lightning deals!</p>
        </div>
      )}
    </div>
  )
